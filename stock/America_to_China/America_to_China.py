...
# America to China.
# 尝试分析纳斯达克指数变动对于A股的影响
# 美国的23日15点收盘对应与中国24日凌晨3点
# 因此发生顺序为 中国23日 -> 美国23日 -> 中国24日
...
def get_data():
	...
	for i in range(len(data)): # For all stock.
		...
		if len(data[i]) > 1 and data[i][0]['symbol'] == 'SHSE.000001':
			szzs = data[i]
	# szzs 数据长度为1946
	csv_reader = csv.reader(open("../NASDAQ.csv"))
	# NASDAQ.csv: ['2021-07-22', '14652.719727', '14694.190430', '14617.870117', '14684.599609',
	# '14684.599609', '3574560000']
	# szzs: {'symbol': 'SHSE.000001', 'open': 2084.84912109375, 'close': 2074.4189453125, 'high': 2096.259033203125,
	# 'low': 2063.089111328125, 'amount': 44793732470.0, 'volume': 5513161200, 
	# 'eob': datetime.datetime(2012, 10, 8, 0, 0, tzinfo=tzfile('PRC'))}
	i = 0
	for line in csv_reader:
		if line[0] == 'Date':
			continue
		nasdaq_day = datetime.datetime.strptime(line[0], "%Y-%m-%d" ).date()
		szzs_day = szzs[i]['eob'].date()
		while szzs_day < nasdaq_day:
			i += 1
			szzs_day = szzs[i]['eob'].date()
			if nasdaq_day == szzs_day:
				szzs[i]['nasdaq_close'] = line[]
